...
import { AddRecruiterDialog } from '@/components/recruiters/AddRecruiterDialog';
...
const RecruiterManagement = () => {
  ...
  const { 
    ...
    handleToggleStatus 
  } = useRecruiterManagement();

  // Filter and conditionally sort recruiters BEFORE pagination
  const filteredAndSortedRecruiters = useMemo(() => {
    const filtered = recruiters.filter(recruiter => {
      const matchesSearch = searchTerm === '' || recruiter.name.toLowerCase().startsWith(searchTerm.toLowerCase());
      
      if (activeTab === 'active') return matchesSearch && recruiter.isActive;
      if (activeTab === 'inactive') return matchesSearch && !recruiter.isActive;
    ...
    return filtered;
  }, [recruiters, searchTerm, activeTab, sortingEnabled]);

  // Pagination calculations
  const totalPages = Math.ceil(filteredAndSortedRecruiters.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const paginatedRecruiters = filteredAndSortedRecruiters.slice(startIndex, endIndex);
        ...
        <div className="rounded-lg border bg-card">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
                <TabsList className="bg-background border">
                  <TabsTrigger value="all" className="flex-1">All Recruiters</TabsTrigger>
          ...
          <div className="flex justify-between items-center px-6 py-3 border-b bg-muted/20">
            <div className="text-sm text-muted-foreground">
              Showing {startIndex + 1}-{Math.min(endIndex, filteredAndSortedRecruiters.length)} of {filteredAndSortedRecruiters.length} recruiters
